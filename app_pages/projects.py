...
def _delete_project_dialog(project_id, project_name):
    st.write(f"Delete project **'{project_name}'**? Runs will not be deleted.")
    ...
    with st.container(border=True):
        ...
        with st.container(horizontal=True, horizontal_alignment="right"):
            with st.popover("Actions", key=f"proj_act_{project['id']}"):
                if st.button("Edit", key=f"proj_edit_{project['id']}"):
                    st.session_state["_edit_project_id"] = project["id"]
                    st.rerun()
                if st.button("Delete", key=f"proj_del_{project['id']}", type="secondary"):
        ...
        if run_indices:
            runs_exp = st.expander(f"Associated Runs ({len(run_indices)})", expanded=False)
            if runs_exp.open:
                with runs_exp:
...
def _render_delete_confirm(project_id: str) -> None:
    ...
    if not project:
        ...
        st.rerun()
        return
    _delete_project_dialog(project_id, project["name"])
